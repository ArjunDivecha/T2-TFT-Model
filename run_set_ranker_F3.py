...
def build_dataset(t60: pd.DataFrame, t2: pd.DataFrame, include_cols: Optional[List[str]] = None) -> DataBundle:
    ...
    # Realized previous cumulative returns (3, 6, 12 months)
    t2_curr_on_merged = pd.merge(pd.DataFrame({'Date': dates}), t2[['Date'] + factors], on='Date', how='left')
    R_df = t2_curr_on_merged[factors]

    # Calculate cumulative returns using a rolling product on (1+R)
    R_plus_1 = R_df + 1
    realized_3m = R_plus_1.rolling(window=3, min_periods=1).apply(np.prod, raw=True) - 1
    realized_6m = R_plus_1.rolling(window=6, min_periods=1).apply(np.prod, raw=True) - 1
    realized_12m = R_plus_1.rolling(window=12, min_periods=1).apply(np.prod, raw=True) - 1

    # Lag by 1 month
    realized_prev_3m = realized_3m.shift(1).fillna(0.0).to_numpy(float)
    realized_prev_6m = realized_6m.shift(1).fillna(0.0).to_numpy(float)
    realized_prev_12m = realized_12m.shift(1).fillna(0.0).to_numpy(float)
    ...
    features = np.stack([F_f, z, rank_pct, realized_prev_3m, realized_prev_6m, realized_prev_12m], axis=2).astype(np.float32)
...
class TemporalSetRanker(nn.Module):
    def __init__(self, n_factors: int, input_size=6, k_history=24, gru_hidden=64, n_heads=4, d_model=64, emb_dim=32, dropout=0.1):
        super().__init__()
...
def main():
    ...
    ap.add_argument('--output_dir', type=str, default='output_F3', help='Directory to save all outputs (predictions, plots, models).')
    ap.add_argument('--out_xlsx', type=str, default='predictions_f3.xlsx', help='Output Excel file name (relative to output_dir).')
    ap.add_argument('--out_pdf', type=str, default='performance_f3.pdf', help='Output PDF file name (relative to output_dir).')
    ...
    for test_t in pbar_oos:
        ...
        model = TemporalSetRanker(n_factors=M, input_size=6, k_history=k_hist, gru_hidden=args.hidden, n_heads=args.heads, d_model=args.hidden, emb_dim=args.emb_dim, dropout=0.1).to(device)
