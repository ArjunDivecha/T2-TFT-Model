...
class SetRankerWithExternal(nn.Module):
    def __init__(self, n_factors: int, input_size=4, f_ext=9, k_history=24, gru_hidden=64, n_heads=4, d_model=64, emb_dim=32, dropout=0.1):
        super().__init__()
        self.k_history = k_history
        self.f_ext = f_ext
        
        # Factor temporal encoder (existing)
        self.factor_gru = nn.GRU(input_size, gru_hidden, num_layers=2, dropout=dropout if 2 > 1 else 0.0, batch_first=True)
        
        # NEW: External context encoder
        if f_ext > 0:
            self.external_gru = nn.GRU(f_ext, gru_hidden//2, num_layers=1, batch_first=True)
    ...
    def forward(self, x_seq, factor_ids, external_seq=None):  # x_seq: [B, N, K, F], external_seq: [B, K, F_ext]
        ...
        # Process external history if available: [B, K, F_ext] -> [B, gru_hidden//2]
...
def main():
    ...
    ap.add_argument('--output_dir', type=str, default='output_F1_external', help='Directory to save all outputs (predictions, plots, models).')
    ap.add_argument('--out_xlsx', type=str, default='predictions_f1_external.xlsx', help='Output Excel file name (relative to output_dir).')
    ap.add_argument('--out_pdf', type=str, default='performance_f1_external.pdf', help='Output PDF file name (relative to output_dir).')
